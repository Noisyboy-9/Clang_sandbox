...
void help();
void create_board();
void show_col_and_row_names();
void insert(char player, char row, char column);
int occupied(char row, char column);
void reset();
bool is_placeable(char player, char row, char column);
...
void show_placeable(char player)
{
    ...
    for (int row = 1; row <= SIZE; row++)
    {
        ...
        {
            if (is_placeable(player, row, column) && board[row][column] == '_')
            {
                board[row][column] = (char)START_ASCII_CODE;
            }
        }
    }
}
